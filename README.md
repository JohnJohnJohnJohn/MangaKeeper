...
4. **Standardize** — Converts page images to **WebP** (original filenames preserved, default quality 95, full resolution). Folder names are cleaned to a standard convention: bracket variants normalized to ASCII `[]` / `()`, language tags shortened (`chinese` → `[chn]`, `english` → `[eng]`), metadata/translator tags removed (for example `[digital]`, `[decensored]`, `[xxx汉化]`), remaining bracket tags lowercased, manga titles title-cased (for example `DUAL LOVE PART` → `Dual Love Part`), no double spaces, spaces around bracket groups, duplicate markers removed. Archives are extracted into a standardized folder. Converted page originals are moved to trash by default.
